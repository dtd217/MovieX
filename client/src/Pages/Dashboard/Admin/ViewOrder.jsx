            ...
            <div className='overflow-x-auto flex flex-col gap-y-2'>
               <div className='border-gray-300 border-2 rounded-md transitions text-black bg-gray-100 overflow-x-auto w-[640px] min-w-full'>
                  <dl className="flex items-center border-gray-900 p-4 justify-start">
                     ...
                     <div className='ml-6'>
                        <dt className="font-semibold text-lg">Tên người dùng</dt>
               ...
               <div className='border-gray-300 border-2 rounded-md transitions text-black bg-gray-100 overflow-x-auto w-[640px] min-w-full'>
                  <dl className="flex items-center border-gray-900 p-4 justify-between">
                     <div>
                        ...
                        <dd className="mt-1">{order?._id ? (order?._id).slice(0, 8) : 'MovieX'}</dd>
                     </div>
                     <div>
                  ...
                  <ul>
                     {order?.orderItems?.length > 0 && order?.orderItems?.map((item) => (
                        <li className="p-4 sm:p-6 border-t-2 border-gray-400" key={item?._id}>
                           <div className="flex items-center sm:items-start">
                              <div className="sm:w-32 sm:h-48 w-28 h-36 shrink-0 overflow-hidden rounded-lg">
                                 <img src={item?.image} alt={item?.title} className="size-full object-cover object-center" />
                              </div>
                              <div className="ml-6 sm:h-48 h-36 flex flex-col sm:justify-between justify-start">
                                 <div>
                                    <h5 className='font-bold sm:text-xl text-lg'>{item?.title}</h5>
                                    <p className="mt-2 sm:block hidden">{`${item?.desc}`.substring(0, 210).slice(0) + " . . ."}</p>
                                 </div>
                                 <p className="sm:block sm:mt-2 sm:text-xl text-lg font-bold">${item?.price}</p>
                              </div>
                           </div>
                        </li>
                     ))}
